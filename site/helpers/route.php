...
/**
 * Shadowsocks Route Helper
 **/
abstract class ShadowsocksHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Cron
	 */
	public static function getCronRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			...
			// Create the link
			$link = 'index.php?option=com_shadowsocks&view=cron&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_shadowsocks&view=cron';
		}
		if ($catid > 1)
		{
			$categories = JCategories::getInstance('shadowsocks.cron');
	...
	/**
	 * Get the URL route for shadowsocks category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_shadowsocks&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_shadowsocks');
		...
		if ($active
			&& $active->component == 'com_shadowsocks'
