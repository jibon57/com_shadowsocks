 ...
 * Servers Table class
 */
class ShadowsocksTableServer extends JTable
{
	/**
	...
	{
		parent::__construct('#__shadowsocks_server', 'id', $db); 
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('server', 'ShadowsocksTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SHADOWSOCKS_SERVER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_shadowsocks.server.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_shadowsocks.server.'.(int) $this->$k;
	}
