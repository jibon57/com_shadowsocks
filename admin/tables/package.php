 ...
 */
class ShadowsocksTablePackage extends JTable
{
	/**
	...
	{
		parent::__construct('#__shadowsocks_package', 'id', $db); 
	}	
	...
	/**
	 * Overload the store method for the Package table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New package. A package created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('package', 'ShadowsocksTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SHADOWSOCKS_PACKAGE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_shadowsocks.package.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_shadowsocks.package.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_shadowsocks');
