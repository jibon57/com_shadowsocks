 ...
 * Users Table class
 */
class ShadowsocksTableUser extends JTable
{
	/**
	...
	{
		parent::__construct('#__shadowsocks_user', 'id', $db); 
	}	
	...
	/**
	 * Overload the store method for the User table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('user', 'ShadowsocksTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SHADOWSOCKS_USER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_shadowsocks.user.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_shadowsocks.user.'.(int) $this->$k;
	}
