 ...
 * Server View class
 */
class ShadowsocksViewServer extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = ShadowsocksHelper::getActions('server',$this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_SHADOWSOCKS_SERVER_NEW' : 'COM_SHADOWSOCKS_SERVER_EDIT'), 'pencil-2 article-add');
		...
		$help_url = ShadowsocksHelper::getHelpUrl('server');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_SHADOWSOCKS_SERVER_NEW' : 'COM_SHADOWSOCKS_SERVER_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_shadowsocks/assets/css/server.css", (ShadowsocksHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css'); 
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_shadowsocks/views/server/submitbutton.js", (ShadowsocksHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
